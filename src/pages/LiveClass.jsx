...
import { startAudioRecording, stopAudioRecording, cancelAudioRecording, uploadAudioToSupabase, updateSessionRecordingPath } from '../lib/audioRecorder';
...
function LiveClass() {
	...
	const [aiInsight, setAiInsight] = useState("Students are showing slight confusion about the current concept. Consider a quick re-explanation of the core principle.");
	...
	const sendBroadcast = (event, payload) => {
		...
		if (!liveChannelRef.current) {
			...
			console.warn(`[LiveClass] Cannot broadcast ${event}: channel not initialized`);
		...
		// Use send() for Realtime-only broadcast (avoids deprecation warning)

		// Supabase optimizes delivery via Realtime if available
	...
	useEffect(() => {
		...
		return () => {
			...
			if (isAudioRecording) {
				...
				console.log('[LiveClass] Component unmounting - stopping audio recording');
		...
		return () => {
			...
			if (currentLiveSessionIdRef.current) {
				...
				supabase
					...
					.then(() => console.log('[LiveClass] Auto-ended session on unmount:', currentLiveSessionIdRef.current))

					.catch(err => console.warn('[LiveClass] Could not auto-end session:', err));
		...
		const channel = supabase
			...
			.on('postgres_changes', {
				...
				if (payload.new.session_id === currentLiveSessionId) {
					...
					console.log('[Doubt] postgres_changes INSERT received:', payload.new);

					// Check if doubt already exists to prevent duplicates

					setDoubts(prev => {

						const existingDoubt = prev.find(d => d.id === payload.new.id);

						if (existingDoubt) {

							console.log('[Doubt] Skipping duplicate from postgres_changes:', payload.new.id);
	...
	useEffect(() => {
		...
		// Prevent redundant initializations if already set up for this group
		...
		const checkHost = async () => {
			...
			try {
				...
				const { data } = await supabase.from('groups').select('created_by').eq('id', id).single();
		...
		const channel = supabase.channel(`live_session_${id}`, {
			...
			.on('broadcast', { event: 'transcript_chunk' }, (payload) => {
				...
				setTranscript((prev) => prev + (payload.payload?.text || ''));
			...
			.on('broadcast', { event: 'ai_update' }, (payload) => {
				...
				if (payload.confusionLevel !== undefined) setConfusionLevel(payload.confusionLevel);

				if (payload.topicClarity) setTopicClarity(payload.topicClarity);
			...
			.on('broadcast', { event: 'live_session_ended' }, (payload) => {
				...
				setActiveLiveSessions(prev => prev.filter(s => s.id !== payload.sessionId));

			})

			.on('broadcast', { event: 'poll_response' }, (payload) => {

				setPollResponses(prev => [...prev, payload]);

				if (isHostRef.current) {

					setConfusionLevel(prev => {

						const newLevel = (payload.answer === 'No' || payload.answer.toLowerCase().includes('confused'))
			...
			.on('broadcast', { event: 'doubt_raised' }, (msg) => {
				...
				console.log('[Doubt] Checking session match:', payload.live_session_id, '===', currentLiveSessionIdRef.current);
				...
				if (payload.live_session_id === currentLiveSessionIdRef.current) {
					...
					setDoubts(prev => {
						...
						const existingDoubt = prev.find(d => d.id === payload.id);

						if (existingDoubt) {

							console.log('[Doubt] Skipping duplicate from broadcast:', payload.id);
			...
			.subscribe((status) => {
				...
				if (status === 'CHANNEL_ERROR') {
					...
					console.error('[LiveClass] Channel subscription failed. Checking if Realtime is enabled in Supabase.');
	...
	useEffect(() => {
		...
		console.log('[Student] Subscribing to transcript_chunks for session:', currentLiveSessionId);
		...
		const transcriptChannel = supabase
			...
			.on(
				...
				(payload) => {
					...
					console.log('[Student] Received transcript from database:', chunk?.text?.substring(0, 50));
			...
			.subscribe((status) => {
				...
				console.log(`[Student] Transcript channel ${currentLiveSessionId} status:`, status);
	...
	useEffect(() => {
		...
		startLiveTranscription(apiKey, {
			...
			onFinal: (text) => {
				...
				const normalize = (s) => s.replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim();
				...
				if (lastNormalized === currentNormalized && lastNormalized.length > 0) {
					...
					console.log('[Teacher] Skipping duplicate/formatted version:', finalText.substring(0, 50));
				...
				console.log('[Teacher] Broadcasting transcript:', cleanChunk.substring(0, 50));

				sendBroadcast('transcript_chunk', { text: cleanChunk });
				...
				if (currentLiveSessionId) {
					...
					(async () => {
						...
						if (error) {
							...
							console.error('[Teacher] Failed to save transcript chunk to DB:', error);

						} else {

							console.log('[Teacher] Transcript chunk saved to DB:', cleanChunk.substring(0, 30));
	...
	const analyzeTranscript = async (textSegment) => {
		...
		if (currentTopic === 'General Discussion') {
			...
			const topic = await aiService.generateSummary(`Identify the main academic topic in 3 words: "${textSegment}"`);
		...
		if (cues || newTopic !== currentTopic) {
			...
			const cueList = cues ? cues.split('\n').filter(line => line.length > 5).slice(0, 2) : teachingCues;
	...
	const handleStartMeeting = async () => {
		...
		console.log("[handleStartMeeting] Starting meeting. isHost:", isHost, "isHostRef:", isHostRef.current);
		...
		if (!isHostRef.current) {
			...
			console.log("[handleStartMeeting] Setting isHost to true since we're creating the meeting");
		...
		if (currentLiveSessionId) {
			...
			try {
				...
				console.log('[handleStartMeeting] Marked teacher_joined=true for session:', currentLiveSessionId);

				// Notify students that session is now visible

				sendBroadcast('live_session_started', { sessionId: currentLiveSessionId, topic: currentTopic, meetingUrl: createdMeetingUrl });

			} catch (err) {

				console.error('[handleStartMeeting] Failed to mark teacher_joined:', err);
	...
	const handleJoinMeeting = () => { setJoinUrlInput(''); setModal('join'); };
	...
	const handleJoinWithUrl = async () => {
		...
		try {
			...
			console.log('[LiveClass] Could not find session by URL (non-critical):', err);
		...
		try {
			...
			if (role === 'teacher') {
				...
				try {
					...
					console.log('[LiveClass] Starting teacher audio recording for session:', data.id);
					...
					console.error('[LiveClass] Failed to start audio recording:', audioErr);
					...
					alert('Warning: Could not start audio recording. Class will continue without recording: ' + audioErr.message);
			...
			sendBroadcast('live_session_started', { sessionId: data.id, topic: classTopic, meetingUrl: url });
		...
		try {
			...
			const { error } = await supabase
				...
				.insert({
					...
					student_name: userData?.full_name || userData?.name || userData?.email || 'Unknown'
	...
	const handleEndClass = async () => {
			...
			try {
				...
				if (audioBlob && audioBlob.size > 0) {
					...
					if (uploadResult) {
						...
						console.log('[LiveClass] Recording uploaded successfully:', uploadResult.path);
				...
				alert('Warning: Could not finalize audio recording. Class will be archived without recording: ' + audioErr.message);
		...
		console.log("Generating final summary from transcript:", transcript.substring(0, 100) + "...");



		const finalSummary = await aiService.generateSummary(transcript || "No transcript available.");
		...
		try {
			...
			if (currentLiveSessionId) {
				...
				sendBroadcast('live_session_ended', { sessionId: currentLiveSessionId });

				console.log('[handleEndClass] Marked live_session as ended:', currentLiveSessionId);
			...
			if (uploadedRecordingPath && currentLiveSessionId) {
				...
				await updateSessionRecordingPath(supabase, currentLiveSessionId, uploadedRecordingPath);
		...
		return (
			...
			<div className="flex-1 p-4 overflow-hidden bg-slate-50">
				...
				<div className="flex flex-col items-center justify-center min-h-[calc(100vh-100px)] gap-6 animate-fade-in">
					...
					<h1 className="m-0 text-4xl text-[#1976d2] font-bold text-center">EduSense Live Intelligence</h1>
					...
					<p className="m-0 text-[#666] text-xl max-w-xl text-center">AI-Powered Live Classrooms. Tracks understanding, not just attendance.</p>
						...
						<div className="w-full max-w-4xl mt-8">
							...
							{activeLiveSessions.length > 0 ? (
								...
								<div className="space-y-4">
									...
									<h2 className="text-xl font-bold text-gray-800 mb-4">Active Live Classes</h2>
								...
								<div className="text-center py-12">
									...
									<h3 className="text-lg font-semibold text-gray-700 mb-2">No Active Live Classes</h3>



									<p className="text-gray-500">When your teacher starts a live class, it will appear here automatically.</p>
				...
				{modal === 'create' && (
					...
					<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-1000" onClick={() => setModal(null)}>
						...
						<div className="bg-white rounded-2xl p-8 min-w-112.5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
							...
							<h3 className="text-[#1976d2] font-bold text-2xl mb-4">Classroom Ready</h3>
							...
							<div className="flex gap-2 mb-6">
								...
								<input type="text" readOnly value={createdMeetingUrl} className="flex-1 p-4 border bg-gray-50 rounded-xl" />
								...
								<button className="px-6 font-bold bg-gray-100 rounded-xl" onClick={handleCopyMeetingUrl}>Copy</button>
							...
							<button className="w-full p-4 bg-green-600 text-white font-bold rounded-xl" onClick={handleStartMeeting}>Enter Classroom</button>
				...
				{modal === 'join' && (
					...
					<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-1000" onClick={() => setModal(null)}>
						...
						<div className="bg-white rounded-2xl p-8 min-w-112.5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
							...
							<h3 className="text-[#1976d2] font-bold text-2xl mb-4">Join Classroom</h3>
							...
							<input type="text" value={joinUrlInput} onChange={e => setJoinUrlInput(e.target.value)} placeholder="Enter Link..." className="w-full p-4 border rounded-xl mb-6" />
							...
							<button className="w-full p-4 bg-blue-600 text-white font-bold rounded-xl" onClick={handleJoinWithUrl}>Join Now</button>
				...
				{modal === 'setup' && (
					...
					<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-1000" onClick={() => setModal(null)}>
						...
						<div className="bg-white rounded-2xl p-8 min-w-112.5 max-w-125 shadow-2xl" onClick={(e) => e.stopPropagation()}>
							...
							<div className="flex items-center gap-3 mb-6">
								...
								<h3 className="text-[#1976d2] font-bold text-2xl">Create Live Class</h3>
							...
							<div className="space-y-4">
								...
								<div>
									...
									<div className="flex items-center gap-3">
										...
										<span className="w-16 text-center font-bold text-blue-600 bg-blue-50 px-3 py-2 rounded-lg">{classDuration}</span>
				...
				{modal === 'rollnumber' && selectedSession && (
					...
					<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-1000" onClick={() => setModal(null)}>
						...
						<div className="bg-white rounded-2xl p-8 min-w-100 max-w-112.5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
							...
							<div className="flex items-center gap-3 mb-6">
								...
								<div>
									...
									<h3 className="text-gray-900 font-bold text-xl">Join Class</h3>
							...
							<div className="mb-6">
								...
								<label className="block text-sm font-bold text-gray-700 mb-2">Enter Your Roll Number *</label>
								...
								<p className="text-xs text-gray-500 mt-2">Your roll number will be recorded for attendance tracking.</p>
				...
				<div className="flex items-center gap-4">
					...
					{role === 'teacher' ? (
						...
						<>
							...
							<div className={`flex items-center gap-2 px-4 py-2 rounded-lg ${isAudioRecording
								...
								<div className={`w-3 h-3 rounded-full ${isAudioRecording ? 'bg-red-500 animate-pulse' : 'bg-gray-400'

									}`}></div>

								<span className={`text-sm font-semibold ${isAudioRecording ? 'text-red-700' : 'text-gray-600'
								...
								{audioRecordingStatus === 'Uploading' && (
									...
									<span className="text-xs text-blue-600 ml-2 animate-pulse">Uploading...</span>
			...
			<div className="flex gap-4 h-[calc(100%-70px)]">
					...
					{role === 'teacher' ? (
						...
						<>
							...
							<div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
								...
								<div className="flex items-center justify-between mb-4">
									...
									<h3 className="m-0 text-gray-900 text-lg font-bold flex items-center gap-2">
										...
										<span className="p-1.5 bg-blue-100 rounded-lg text-blue-600">📊</span> Intelligence Dashboard
								...
								<div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100/50 mb-5">
									...
									<p className="m-0 text-xs font-bold text-blue-400 uppercase tracking-widest mb-1">Current Topic</p>



									<p className="m-0 text-blue-900 font-bold text-base truncate">{currentTopic}</p>
								...
								<div className="mb-6">
									...
									<div className="flex justify-between items-end mb-2">
										...
										<span className="text-sm font-bold text-gray-700">Confusion Meter</span>



										<span className={`text-xs font-bold uppercase ${confusionLevel > 50 ? 'text-red-500' : confusionLevel > 20 ? 'text-orange-500' : 'text-green-500'}`}>



											{confusionLevel > 50 ? 'High Confusion' : confusionLevel > 20 ? 'Caution' : 'Low Confusion'}
									...
									<div className="flex gap-1.5 h-12 items-end">
										...
										{[1, 2, 3, 4, 5].map((bar) => {
											...
											return (
												...
												<div key={bar} className={`flex-1 rounded-t-md transition-all duration-500 ${color}`} style={{ height: isActive ? `${20 * bar}%` : '15%' }}></div>
									...
									<div className="mt-2 text-center"><span className="text-2xl font-black text-gray-800">{confusionLevel}%</span></div>
								...
								<div className="grid grid-cols-2 gap-3 mb-6">
									...
									<div className="bg-gray-50 p-3 rounded-xl border border-gray-100">
										...
										<p className="m-0 text-[10px] font-bold text-gray-400 uppercase mb-1">Poll Accuracy</p>
										...
										<p className="m-0 text-base font-bold text-gray-800">
											...
											{pollResponses.length > 0 ? Math.round((pollResponses.filter(r => r.answer.includes('Yes') || r.answer.includes('clear')).length / pollResponses.length) * 100) : 0}%
									...
									<div className="bg-gray-50 p-3 rounded-xl border border-gray-100">
										...
										<p className="m-0 text-[10px] font-bold text-gray-400 uppercase mb-1">Doubts</p>



										<p className="m-0 text-base font-bold text-gray-800">{doubts.length}</p>
									...
									<div className="bg-gray-50 p-3 rounded-xl border border-gray-100">
										...
										<p className="m-0 text-[10px] font-bold text-gray-400 uppercase mb-1">Transcription</p>
										...
										<p className="m-0 text-xs font-bold text-gray-800 flex items-center gap-1">
											...
											<span className={`w-2 h-2 rounded-full ${transcriptionStatus === 'Live' ? 'bg-green-500 animate-pulse' : 'bg-red-400'}`}></span>
									...
									<div className="bg-gray-50 p-3 rounded-xl border border-gray-100">
										...
										<p className="m-0 text-[10px] font-bold text-gray-400 uppercase mb-1">Response Delay</p>



										<p className="m-0 text-base font-bold text-gray-800">1.2s</p>
								...
								{isHost && (
									...
									<button onClick={handleLaunchPoll} className="w-full py-3.5 bg-green-500 hover:bg-green-600 text-white font-bold rounded-xl shadow-lg shadow-green-200 transition-all flex items-center justify-center gap-2">
							...
							<div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
								...
								<h4 className="m-0 mb-4 text-gray-800 text-sm font-bold uppercase tracking-wider">Topic-wise Clarity</h4>
									...
									{topicClarity.map((topic, idx) => (
										...
										<div key={idx}>
											...
											<div className="flex justify-between items-center mb-1.5">
												...
												<span className="text-xs font-medium text-gray-600 truncate mr-4">{idx + 1}. {topic.name}</span>



												<span className={`text-xs font-bold ${topic.clarity > 80 ? 'text-green-500' : topic.clarity > 50 ? 'text-orange-500' : 'text-red-500'}`}>{topic.clarity}%</span>
											...
											<div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
												...
												<div className={`h-full transition-all duration-700 ${topic.clarity > 80 ? 'bg-green-500' : topic.clarity > 50 ? 'bg-orange-500' : 'bg-red-500'}`} style={{ width: `${topic.clarity}%` }}></div>
							...
							<div className="bg-indigo-50/50 rounded-2xl p-5 shadow-sm border border-indigo-100/50 border-dashed relative overflow-hidden shrink-0">
								...
								<h4 className="m-0 mb-3 text-indigo-900 text-xs font-bold uppercase tracking-widest">AI PEDAGOGICAL INSIGHT</h4>



								<p className="m-0 text-indigo-800 text-sm font-medium leading-relaxed mb-4">{aiInsight || "Analyzing lecture flow for pedagogical suggestions..."}</p>
								...
								{teachingCues.length > 0 && (
									...
									<div className="mb-4 bg-white/50 p-3 rounded-xl border border-indigo-100/50">
										...
										<p className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-2">Key Highlights & Cues</p>
										...
										<ul className="m-0 pl-4 space-y-1">
											...
											{teachingCues.map((cue, idx) => (
												...
												<li key={idx} className="text-xs text-indigo-700 font-medium">{cue}</li>
								...
								{isHost && aiInsight && (
									...
									<button onClick={() => { setAiInsight("Suggestion acknowledged. Monitoring student clarity..."); setTeachingCues([]); }} className="w-full py-2.5 bg-indigo-600 text-white text-xs font-bold rounded-lg hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-100 active:scale-95">
								...
								<details className="group" open>
									...
									<summary className="p-4 bg-gray-50/50 cursor-pointer flex items-center justify-between list-none">
										...
										<h4 className="m-0 text-gray-800 text-xs font-bold uppercase tracking-wider flex items-center gap-2">
											...
											<span className={`text-[9px] px-1.5 py-0.5 rounded-full ${transcriptionStatus === 'Live' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{transcriptionStatus}</span>
									...
									<div className="p-4 max-h-55 overflow-y-auto text-sm text-gray-700 leading-relaxed bg-white border-t border-gray-50 scrollbar-thin">
										...
										{interimTranscript && <span className="opacity-70 italic"> {interimTranscript}</span>}
										...
										{!transcript && !interimTranscript && (
											...
											<span className="text-gray-500 italic">
												...
												{transcriptionStatus === 'Live' ? 'Speak into your microphone — text will appear here.' : transcriptionStatus === 'Error' ? 'Transcription error. Check mic permission for this tab (EduSense).' : 'Waiting for speech...'}
							...
							<div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex-1 flex flex-col min-h-50">
								...
								<div className="flex items-center justify-between mb-4">
									...
									<h4 className="m-0 text-gray-800 text-sm font-bold uppercase tracking-wider">Student Doubts</h4>



									{doubts.length > 0 && <span className="px-2 py-0.5 bg-red-100 text-red-600 rounded text-[10px] font-bold">{doubts.length}</span>}
								...
								<div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
									...
									{doubts.length === 0 ? (
										...
										doubts.map((d) => (
											...
											<div key={d.id} className="bg-gray-50 border border-gray-100 p-3.5 rounded-xl">



												<p className="m-0 text-sm text-gray-800 font-medium leading-snug lowercase first-letter:uppercase">"{d.text}"</p>



												<span className="text-[10px] text-gray-400">Anonymous • {Math.round((new Date() - d.timestamp) / 60000)}m ago</span>
						...
						<>
							...
							<div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
								...
								<h3 className="m-0 text-gray-900 text-base font-bold flex items-center gap-2 mb-1">
									...
									<span className="p-1.5 bg-blue-100 rounded-lg text-blue-600">📜</span> Teacher&apos;s Live Transcription
								...
								<p className="m-0 text-[10px] text-gray-500 mb-3 flex items-center gap-1.5">
									...
									<span className={`w-1.5 h-1.5 rounded-full ${transcriptionStatus === 'Live' ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`}></span>



									{transcriptionStatus === 'Live' ? 'Live' : 'Waiting for teacher…'}
								...
								<div className="p-4 max-h-55 overflow-y-auto text-sm text-gray-700 leading-relaxed bg-gray-50/50 rounded-xl border border-gray-100 custom-scrollbar">
									...
									{!transcript && !interimTranscript && (
										...
										<span className="text-gray-500 italic">
											...
											{transcriptionStatus === 'Live' ? 'Teacher is speaking...' : transcriptionStatus === 'Error' ? 'Transcription error on teacher side.' : 'Waiting for teacher to start speaking...'}
							...
							<div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 shrink-0">
								...
								<div className="p-4 bg-amber-50/50 rounded-xl border border-amber-100/50 min-h-20">
									...
									{liveSummaryLoading ? (
										...
										<p className="m-0 text-sm text-amber-700/80">Updating summary…</p>
										...
										<p className="m-0 text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{liveSummary}</p>
										...
										<p className="m-0 text-sm text-gray-500 italic">Summary will appear as the class progresses (after enough speech).</p>
							...
							<div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex-1 flex flex-col min-h-70">
								...
								<div className="flex items-center justify-between mb-4">
									...
									<h4 className="m-0 text-gray-800 text-sm font-bold uppercase tracking-wider">My doubts</h4>



									{doubts.length > 0 && <span className="px-2 py-0.5 bg-blue-100 text-blue-600 rounded text-[10px] font-bold">{doubts.length}</span>}
								...
								<div className="flex-1 overflow-y-auto space-y-3 pr-2 mb-4 custom-scrollbar">
									...
									{doubts.length === 0 ? (
										...
										<div className="h-full flex flex-col items-center justify-center text-gray-400 opacity-60">
											...
											<p className="text-xs">No doubts raised yet. Ask anything anonymously below.</p>
										...
										doubts.map((d) => (
											...
											<div key={d.id} className="bg-gray-50 border border-gray-100 p-3 rounded-xl">



												<p className="m-0 text-sm text-gray-800 font-medium leading-snug">"{d.text}"</p>



												<span className="text-[10px] text-gray-400">{Math.round((new Date() - d.timestamp) / 60000)}m ago</span>
				...
				{import.meta.env.DEV && (
					...
					<div className="fixed bottom-4 left-4 z-9999 bg-black/80 text-white p-3 rounded-lg text-[10px] font-mono pointer-events-none opacity-50 hover:opacity-100 transition-opacity">
						...
						<p className="m-0 mb-1 font-bold text-yellow-400 uppercase tracking-tighter">Debug</p>



						<p className="m-0">Host: {isHost ? "Y" : "N"} · Rec: {isRecording ? "Y" : "N"} · Status: {transcriptionStatus}</p>



						<p className="m-0">Channel: {channelStatus} · Transcript: {transcript?.length || 0} chars</p>
				...
				{activePoll && role === 'student' && (
					...
					<div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-2000 flex items-center justify-center p-6 animate-fade-in">
						...
						<div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl scale-in ring-8 ring-blue-50">
							...
							<h3 className="text-2xl font-black text-gray-900 mb-2 leading-tight">{activePoll.question}</h3>



							<p className="text-gray-500 mb-8 text-sm">Quick response requested by Teacher</p>
